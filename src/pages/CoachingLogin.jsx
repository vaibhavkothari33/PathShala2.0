...
import { ArrowRight, Mail, Lock, School, Users, CheckCircle } from 'lucide-react';
...
const CoachingLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Uncomment this line for actual login
      // const session = await account.createEmailSession(formData.email, formData.password);
      const userData = await account.get();
      
      // Check if user is a coaching entity, if not redirect to registration
      if (!userData.labels || !userData.labels.includes('coaching')) {
        toast.error('Account not registered as coaching institute');
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex">
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-indigo-600 to-blue-700 text-white p-12 flex-col justify-between">
        ...
        <div className="text-sm text-blue-200">
          ...
          <p className="mt-1">Trusted by 500+ coaching institutes</p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md">
          <div className="bg-white p-8 rounded-2xl shadow-xl">
            ...
            <div className="mt-8 text-center">
              ...
              <Link to="/coaching/register" className="font-medium text-indigo-600 hover:text-indigo-500">
                Sign up as a coaching institute
              </Link>
            </div>
          </div>
